...
if(uploaded_file is not None):
    ...
    def ask_question(agent, question_text, subheader, key, weight):
        prompt = f"""
        ...
        if(res.data):
            ...
            if(answer=="yes"): score = 1*weight
            elif (answer=="no"): score = 0*weight
    ...
    agents_info=[
        ...
        (hackathon_agent, "Has the candidate participated in hackathons, competitions, or challenges related to their field?", "hackathon_agent"),
    ...
    if(job_res.data):
        st.subheader("Are there any frequent job changes? A job change can be a change in organization in less than 2 years")
        st.write("Answer: ", job_res.data.answer)
